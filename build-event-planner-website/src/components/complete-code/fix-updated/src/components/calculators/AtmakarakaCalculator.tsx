...
export default function AtmakarakaCalculator({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.birthDate && formData.birthTime && formData.birthPlace) {
      ...
      setResult(`${lang === 'hi' ? 'आत्मकरक ग्रह' : 'Atmakaraka Planet'}: ${randomPlanet}`);
    }
  };
      ...
      <div className="relative h-[32vh] md:h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white mb-3 tracking-tight drop-shadow-xl animate-slideInUp">
            {lang === 'hi' ? 'आत्मकरक और दाराकरक कैल्कुलेटर' : 'Atmakaraka & Darakaraka Calculator'}
          </h1>
        ...
        <div className="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-white/10 p-8">
          {!result ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center"><button type="submit" className="bg-linear-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-gray-800 font-black py-4 px-8 rounded-xl transition-all shadow-xl hover:shadow-amber-500/30 text-lg">{lang === 'hi' ? 'करक ग्रह कैल्कुलेट करें' : 'Calculate Karaka Planets'}</button></div>
            </form>
          ) : (
