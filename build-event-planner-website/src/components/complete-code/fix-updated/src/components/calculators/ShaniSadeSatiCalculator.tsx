...
export default function ShaniSadeSatiCalculator({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.birthDate) {
      ...
      setResult(lang === 'hi' ? `शनि की ${randomPhase} अवस्था` : `Shani in ${randomPhase} phase`);
    }
  };
      ...
      <div className="relative h-[32vh] md:h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white mb-3 tracking-tight drop-shadow-xl animate-slideInUp">
            {lang === 'hi' ? 'शनि साडे साती कैल्कुलेटर' : 'Shani Sade Sati Calculator'}
          </h1>
        ...
        <div className="bg-gray-800/80 backdrop-blur-md rounded-2xl border border-white/10 p-8">
          {!result ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center"><button type="submit" className="bg-linear-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white font-black py-4 px-8 rounded-xl transition-all shadow-xl hover:shadow-gray-500/30 text-lg">{lang === 'hi' ? 'शनि साडे साती जांचें' : 'Check Shani Sade Sati'}</button></div>
            </form>
          ) : (
