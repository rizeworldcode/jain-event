...
export default function TransitChartCalculator({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; }) {
  const { lang } = useContext(LanguageContext);
  const [formData, setFormData] = useState({ name: '', birthDate: '', currentDate: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.name && formData.birthDate && formData.currentDate) {
      setResult(lang === 'hi' ? 'ट्रांजिट चार्ट तैयार किया गया है।' : 'Transit chart generated.');
    }
  };
      ...
      <div className="relative h-[32vh] md:h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white mb-3 tracking-tight drop-shadow-xl animate-slideInUp">
            {lang === 'hi' ? 'ट्रांजिट चार्ट कैल्कुलेटर' : 'Transit Chart Calculator'}
          </h1>
          ...
          {!result ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div><label className="block text-gray-300 font-bold mb-2">{lang === 'hi' ? 'वर्तमान तिथि' : 'Current Date'}</label><input type="date" name="currentDate" value={formData.currentDate} onChange={handleInputChange} className="w-full bg-gray-700 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500/50" required /></div>
              </div>
              <div className="text-center"><button type="submit" className="bg-linear-to-r from-amber-600-500 to-blue-500 hover:from-amber-600-600 hover:to-blue-600 text-white font-black py-4 px-8 rounded-xl transition-all shadow-xl hover:shadow-amber-600-500/30 text-lg">{lang === 'hi' ? 'ट्रांजिट चार्ट बनाएं' : 'Create Transit Chart'}</button></div>
            </form>
          ) : (
            <div className="text-center">
              ...
              <button onClick={() => setResult(null)} className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 px-6 rounded-xl transition-all">{lang === 'hi' ? 'दोबारा बनाएं' : 'Create Again'}</button>
            </div>
          )}
        </div>
      </div>
